...
__global__ void AttentionLossForwardGPU(const int nthreads,
	...
	CUDA_KERNEL_LOOP(i, nthreads) {
		...
		if ( has_ignore_label_ && target_value == ignore_label_ ) {
			...
			//counts[ i ] = 1;
			if (target_value == 1){
				scale[ i ] = (alpha) * powf(beta, powf(fmaxf(1 - (sigmoid_data[ i ]), Dtype(0.00000001)), gamma ) );
				...
				scale[ i ] = (1 - alpha) * powf(beta, powf(fmaxf(1 - (( 1 - sigmoid_data[ i ] ) ), Dtype(0.00000001)), gamma ));
				oriloss[ i ] = -(input_data[ i ] * ( target[ i ] - ( input_data[ i ] >= 0 ) ) -
					log(1 + exp(input_data[ i ] - 2 * input_data[ i ] *
					( input_data[ i ] >= 0 ))));
				counts[ i ] = 1;
...
__global__ void AttentionLossBackwardGPU(const int nthreads, const Dtype* sigmoid_data, const Dtype* target, 
	...
	CUDA_KERNEL_LOOP(i, nthreads) {
		...
		if(target_value == 1){
			Dtype tmp = (-1) * (sigmoid_data[ i ] * oriloss[i] 
				* log(beta) * gamma * powf(fmaxf((1 - sigmoid_data[ i ]), Dtype(0.00000001)), gamma - 1)  + 1);
			bottom_diff[i] = scale[i] * (1 - sigmoid_data[ i ])* tmp;
		}
		else if (target_value == 0){
			Dtype tmp = (1) * (( 1 - sigmoid_data[ i ] ) * oriloss[i] 
				* log(beta) * gamma * powf(fmaxf(sigmoid_data[ i ], Dtype(0.00000001)), gamma - 1)  + 1);
			bottom_diff[i] = scale[i] * (sigmoid_data[ i ]) * tmp;
		}
		else{
			bottom_diff[i] = 0;
		}
